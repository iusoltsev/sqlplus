...
set feedback on heading on timi off pages 200 lines 1000 echo off  VERIFY OFF
...
col V_SQL_Adapt for a11
col XML_Adapt   for a9
col SPD_Valid   for a9
col SPD_Used    for a8
col CF          for a3
col CURSOR_STATUS for a14
col REASON1 for a40
col REASON2 for a40
...
--       s.IS_RESOLVED_ADAPTIVE_PLAN as "ADAPT",
       case when s.IS_RESOLVED_ADAPTIVE_PLAN is null then 'N' when s.IS_RESOLVED_ADAPTIVE_PLAN = 'N' then '' else 'Y' end         as V_SQL_Adapt,
       REGEXP_SUBSTR ( dbms_lob.substr(p.other_xml,4000), '<(info) type="adaptive_plan" note="y">([^<]+)</\1>', 1, 1, NULL, 2)    as XML_Adapt,
...
--       ROLL_INVALID_MISMATCH as "ROLL",
       dbms_lob.substr(REGEXP_SUBSTR ( reason, '<(reason)>([^<]+)</\1>', 1, 1, NULL, 2), 100) reason1,
       dbms_lob.substr(REGEXP_SUBSTR ( reason, '<(reason)>([^<]+)</\1>', 1, 2, NULL, 2), 100) reason2,
...
--       dbms_lob.substr(REGEXP_SUBSTR ( reason, '<(reason)>([^<]+)</\1>', 1, 4, NULL, 2), 100) reason#4,
       trim(dbms_lob.substr(REGEXP_SUBSTR ( reason, '<(_fix_control_key)>([^<]+)</\1>', 1, 1, NULL, 2), 100)) fix_control1,
