...
col request_parent_list for a200
...
--select * from ah order by 11 desc
select--+ leading(q s) parallel(8)
  sql_id, ah.module, ah.action
...
, listagg(distinct s.request_id||'|'||s.parent_request_id, '; ' ON OVERFLOW TRUNCATE '...') as request_parent_list
--, listagg(s.parent_request_id, '; ' ON OVERFLOW TRUNCATE '...')   as parent_request_ids
, CONCURRENT_PROGRAM_ID, n.CONCURRENT_PROGRAM_NAME
, replace(replace(p1.ARGUMENT_TEXT,chr(10),' '),chr(13),' ') as ARGUMENT_TEXT
, min(p1.ACTUAL_START_DATE)      as MIN_ACTUAL_START_DATE
, min(p1.ACTUAL_COMPLETION_DATE) as MIN_ACTUAL_COMPLETION_DATE
--select * 
...
, sum_elap_time, sum_execs, ela_per_cent, cpu_per_cent, reads_per_cent
, CONCURRENT_PROGRAM_ID, n.CONCURRENT_PROGRAM_NAME
, replace(replace(p1.ARGUMENT_TEXT,chr(10),' '),chr(13),' ')
--, p1.ACTUAL_START_DATE, p1.ACTUAL_COMPLETION_DATE
