...
--                    and child_number = (select min(child_number) from v$sql_plan where sql_id = '&&1' and plan_hash_value = nvl('&&2',0)))) d),
                    and (inst_id, child_number) in (select inst_id, child_number from gv$sql_plan where sql_id = '&&1' and plan_hash_value = nvl('&&2', 0) and rownum <= 1))) d),
...
--                    and child_number = (select min(child_number) from v$sql_plan where sql_id = '&&3' and plan_hash_value = nvl('&&4',0)))) d)
                    and (inst_id, child_number) in (select inst_id, child_number from gv$sql_plan where sql_id = '&&3' and plan_hash_value = nvl('&&4', 0) and rownum <= 1))) d)
select * from plh1 full join plh2 on plh1.phv_&&2 = plh2.phv_&&4
  where --not (plh1.phv_&&2 || plh2.phv_&&4 like 'INDEX%'        or       -- may be useful to exclude a lot of non-principal hints
              ...
              --and  
                     plh1.phv_&&2 || plh2.phv_&&4 like '%' || '&&5' || '%'
minus
select * from plh1 join plh2 on plh1.phv_&&2 = plh2.phv_&&4
/
