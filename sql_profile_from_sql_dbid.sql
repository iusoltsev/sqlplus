...
-- Usage: SQL> @sql_profile_from_sql &sql_id &plan_hash_value &dbid "SQL_Profile_name" "SQL_Profile_desc"
--
                    ...
                    OTHER_XML
                     from (select OTHER_XML
                            ...
                            where sql_id = '&&1'
                              and plan_hash_value = nvl('&&2', plan_hash_value)
                              and trim(OTHER_XML) is not null
and SYS_CONTEXT('USERENV','DBID') = nvl('&&3', SYS_CONTEXT('USERENV','DBID'))
                           union all
                           select OTHER_XML
