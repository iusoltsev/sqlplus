...
-- Usage: SQL> @ash_sql_block_tree_hist "program like '%LMS%'" start_snap stop_snap 0
--
...
--       ,sql_plan_operation||' '||sql_plan_options
  ...
  from (select * from dba_hist_active_sess_history where snap_id between &2 and &3)
...
connect by nocycle (ash.SAMPLE_ID       = prior ash.SAMPLE_ID or abs(to_char(ash.sample_time,'SSSSS') - to_char(prior ash.sample_time,'SSSSS')) <= 1)
               ...
               and prior ash.INSTANCE_NUMBER = ash.BLOCKING_INST_ID
 group by LEVEL,
          instance_number,
--          BLOCKING_INST_ID,
        case when REGEXP_INSTR(program, '\([A-Z]...\)') = 0 then '(USER)'
