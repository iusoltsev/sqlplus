...
col USERNAME for a60 head USER_CLIENT_ID
col PRIOGRAM for a18
col P1TEXT for a40
col P2TEXT for a40
col P3TEXT for a40
...
column CHAR_DATE new_value SYS_DATE
...
-- LPAD(' ', (LEVEL - 1) * 2) || 'INST#' || s.inst_id || ' SID#' || sid as BLOCKING_TREE,
 ...
 substr(s.program,1,18) as PROGRAM,
...
-- p3raw
  from s
--join LOCKS l on s.inst_id = l.inst_id and s.sid = l.sid
