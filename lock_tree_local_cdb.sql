...
,BLOCKERS as
 (select distinct L1.inst_id, L1.sid, L1.con_id
...
,WAITERS as (select inst_id, sid, con_id from S where blocking_session is not null or blocking_instance is not null
             union
             select distinct L2.inst_id, L2.sid, L2.con_id
              from LOCKS L1, LOCKS L2
             where L1.block > 0
               and L1.ID1 = L2.ID1
               and L1.ID2 = L2.ID2
               and L2.REQUEST > 0)
--select * from WAITERS--BLOCKERS
...
select--+ opt_param('_connect_by_use_union_all' 'false')
 LPAD(' ', (LEVEL - 1) * 2) || 'INST#' || s.inst_id || ' SID#' || sid || ' CON#' || s.con_id as BLOCKING_TREE,
...
connect by NOCYCLE prior sid = blocking_session and prior s.inst_id = blocking_instance
 start with (s.inst_id, s.sid) --in ((1,4172))
