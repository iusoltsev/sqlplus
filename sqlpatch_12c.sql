--
-- Creates the "special" 12.1 SQL Patch which turns off the 12c Adaptive Features, 12c Adaptive Dynamic Sampling and 11.2 Adaptive Cursor Sharing for given sql_id
-- Usage: SQL> @sqlpatch_12c arazapyc4cj4g
...
begin
  ...
  loop
    sys.dbms_sqldiag_internal.i_create_patch(sql_text  => reco.sql_fulltext,
                                             hint_text => 'opt_param(''optimizer_adaptive_features'' ''false'') no_bind_aware OPT_PARAM(''optimizer_dynamic_sampling'' 0)',
                                             name      => 'NO_ADAPT_&&1');
  end loop;
end;
/
