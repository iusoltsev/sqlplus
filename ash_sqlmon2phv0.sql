...
-- about v$sql_plan.child_number multi and multi/adaptive PHV
  ...
  select distinct 
         sql_id,
         decode(sql_plan_hash_value,0,SQL_FULL_PLAN_HASH_VALUE,sql_plan_hash_value) as plan_hash_value,
         sql_plan_line_id    as id,
         sql_plan_operation  as operation,
         sql_plan_options    as options,
         ''                  as qblock_name,
         ''                  as object_alias,
         LISTAGG(distinct owner,       '; ' ON OVERFLOW TRUNCATE '...') WITHIN GROUP (ORDER BY 1) as object_owner,
         LISTAGG(distinct object_name, '; ' ON OVERFLOW TRUNCATE '...') WITHIN GROUP (ORDER BY 1) as object_name,
...
group by sql_id,
         decode(sql_plan_hash_value,0,SQL_FULL_PLAN_HASH_VALUE,sql_plan_hash_value),
         sql_plan_line_id,
         sql_plan_operation,
         sql_plan_options
)
select 0,'Hard Parse' as LAST_PLSQL, -- the hard parse phase, sql plan does not exists yet, sql_plan_hash_value = 0
       sql_id,
       to_char(sql_plan_hash_value) as plan_hash_value,
...
and not SQL_FULL_PLAN_HASH_VALUE > 0
UNION ALL
select 1,'Soft Parse' as LAST_PLSQL, -- the soft parse phase, sql plan exists but execution didn't start yet, sql_exec_id is null
       sql_id,              
       to_char(sql_plan_hash_value) as plan_hash_value,
       ash_stat.sql_plan_line_id as ID,
--       -1 as ID,
...
UNION ALL
SELECT 2,'Main Query w/o saved plan'       -- direct SQL which plan not in gv$sql_plan, dba_hist_sql_plan (ro-standby)
                                                                 as LAST_PLSQL,
       pt.sql_id                                                 as SQL_ID,
       to_char(pt.plan_hash_value)||decode(ash_stat.sql_plan_hash_value,0,'F','') as plan_hash_value,
       pt.id,
...
----  left -- 2exclude empty
 join ash_stat
  on --pt.parent_id       = -2 and
     ...
     pt.plan_hash_value = decode(ash_stat.sql_plan_hash_value,0,ash_stat.SQL_FULL_PLAN_HASH_VALUE,ash_stat.sql_plan_hash_value) and
...
--ORDER BY PT.ID
UNION ALL
SELECT 4,decode(pt.id, 0, p.object_name||'.'||p.procedure_name, null) as LAST_PLSQL, -- recursive SQLs plan+stats
       decode(pt.id, 0, pt.sql_id, null) as SQL_ID,
       decode(pt.id, 0, to_char(pt.plan_hash_value), null) as plan_hash_value,
       pt.id,
       lpad(' ', 2 * level) || pt.operation || ' ' || pt.options as PLAN_OPERATION,
...
UNION ALL
select 5,'OtherRec.Waits' as LAST_PLSQL, -- non-identified SQL (PL/SQL?) exec stats
       sql_id,
       to_char(ash_stat.sql_plan_hash_value) as sql_plan_hash_value,
       ash_stat.sql_plan_line_id,
...
UNION ALL
select 8,'Recursive SQL by PHV' as LAST_PLSQL, -- Recursive Summary-3
       sql_id,
       to_char(sql_plan_hash_value) as sql_plan_hash_value,--to_number(null) as sql_plan_hash_value,
       to_number(null) as sql_plan_line_id,
...
, sql_plan_hash_value
UNION ALL
select 9,'Recursive SQL Summary' as LAST_PLSQL, -- Recursive Summary-3
       '',
       to_char(null) as sql_plan_hash_value,
       to_number(null) as sql_plan_line_id,
